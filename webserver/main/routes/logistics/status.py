...
@logistics_status_namespace.route("/logistics/v1/status")
class Status(Resource):
    path_schema = get_json_schema_for_given_path('/status', domain="logistics")

    @expects_json(path_schema)
    def post(self):
        response_schema = get_json_schema_for_response('/status', domain="logistics")
